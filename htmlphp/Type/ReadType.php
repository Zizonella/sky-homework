...
$sqll='SELECT * FROM listing_type';


$stat = $conn->prepare($sqll);
$stat ->execute();
$listingType = $stat->fetchAll(PDO::FETCH_OBJ);
?>
...
<div class="container"> 
    <div class="card">
        <h3 class="card-header text-center font-weight-bold text-uppercase py-4">Listing Type</h3>
        <div class="card-body">
            <div id="table" class="table-editable">
                <table class="table table-bordered table-responsive-sm table-striped text-center">
                    ...
                    <tbody>
                        <?php foreach($listingType as $typeOption): ?>
                        <tr>
                            <td class="pt-3-half" contenteditable="true"><?php echo $typeOption->id;?></td>
                            <td class="pt-3-half" contenteditable="true"><?php echo $typeOption->type;?></td>
                            <td>
                                <a href="EditType.php?id= <?= $typeOption->id ?> "<span class="table-remove"><button type="button" class="btn btn-danger btn-rounded btn-sm my-0">Edit</button></span></a>
                            </td>
                            <td>    
